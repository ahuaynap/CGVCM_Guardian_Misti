...
public sealed class GuardianMistiProtectionPlayModeTests
{
    ...
    {
        yield return new EnterPlayMode();UnityEngine.SceneManagement.SceneManager.LoadScene(SceneNames.Level01);yield return null;var look=Object.FindFirstObjectByType<PlayerLookController>();Assert.NotNull(look);Assert.AreEqual(1,PlayerLookController.CountActiveLookControllers(look.gameObject));float yaw=look.Yaw;look.ApplyLookDelta(new Vector2(10f,5f),true,.001f);Assert.AreEqual(10f*look.MouseSensitivity,Mathf.Abs(Mathf.DeltaAngle(yaw,look.Yaw)),.1f);Assert.AreEqual(-5f*look.MouseSensitivity,look.Pitch,.1f);look.ApplyLookDelta(new Vector2(0f,1000f),true,.001f);Assert.That(look.Pitch,Is.InRange(look.MinimumPitch,look.MaximumPitch));var crouch=look.GetComponent<PlayerCrouchController>();float sensitivity=look.MouseSensitivity;crouch.BeginCrouch();Assert.True(crouch.IsCrouching);Assert.AreEqual(sensitivity,look.MouseSensitivity);yield return new ExitPlayMode();
    }
