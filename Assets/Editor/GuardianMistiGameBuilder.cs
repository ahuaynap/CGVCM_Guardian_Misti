...
public static class GuardianMistiGameBuilder
{
    ...
    private static void BuildLevel(bool level2)
    {
        ...
        if(!level2){var quakeDust=Dust(new Vector3(0,2,10),new Vector3(8,2.5f,18),25);var quake=managers.AddComponent<EarthquakeController>();Set(quake,"profile",AssetDatabase.LoadAssetAtPath<EarthquakeProfile>("Assets/ScriptableObjects/Earthquakes/Intermediate.asset"));Set(quake,"cameraEffectRoot",camera.transform);Set(quake,"countdownText",Find(canvas.transform,"EarthquakeCountdown").GetComponent<TextMeshProUGUI>());Set(quake,"intensityText",Find(canvas.transform,"EarthquakeIntensity").GetComponent<TextMeshProUGUI>());Set(quake,"dust",quakeDust);}
    ...
    private static Canvas BuildGameplayHud(out ObjectiveUI objectiveUI,out InventoryUI inventoryUI,out NotificationUI notificationUI,out InteractionUIController interactionUI,out GameObject pause,out GameObject completion,out GameCompletionUI completionUI,out GameCompletionController completionController)
    {
        ...
        Text("EarthquakeIntensity",hud.transform,"Intensidad simulada: Preparación",17,new Vector2(.5f,.86f),new Vector2(520,40),Orange,FontStyles.Bold);Text("EarthquakeCountdown",hud.transform,"El simulacro comenzará en 3...",34,new Vector2(.5f,.68f),new Vector2(850,75),Color.white,FontStyles.Bold);
    ...
    {
        var protection=TriggerMarker("InteriorProtectionZone",new Vector3(-4,1,1.8f),new Vector3(3.5f,2,3));var protectionController=protection.AddComponent<ObjectiveTrigger>();Set(protectionController,"objectiveId",GameIds.Level01Protect);Set(protectionController,"notificationUI",note);SafeZoneVisual(protection.transform,new Vector3(0,-1,0));Sign("ZONA DE PROTECCIÓN",new Vector3(-4,2.6f,2.8f),safeMat);
    ...
    {
        var list=l2?new[]{new Objective(GameIds.Level02CollectRadio,"Encuentra la radio de emergencia."),new Objective(GameIds.Level02CollectAccessKey,"Encuentra la llave de acceso."),new Objective(GameIds.Level02ActivateBeacon,"Activa la baliza de emergencia."),new Objective(GameIds.Level02ReachSafeZone,"Llega a la zona segura.")}:new[]{new Objective(GameIds.Level01Protect,"Protégete durante el sismo."),new Objective(GameIds.Level01ExitRoom,"Sal de la habitación."),new Objective(GameIds.Level01CollectBackpack,"Encuentra la mochila de emergencia."),new Objective(GameIds.Level01ActivateEvacuation,"Activa la salida de evacuación."),new Objective(GameIds.Level01ReachExit,"Dirígete al punto de salida.")};
