...
public static class GuardianMistiGameBuilder
{
    ...
    public static void BuildCompleteGame()
    {
        try
        {
            Debug.Log("[Guardian Misti] Preparing polished assets"); EnsureFolders(); CreateMaterials(); CreateVolumeProfiles(); CreateEarthquakeProfiles(); CreateItems(); CreatePrefabs();
    ...
    private static void EarthquakeAsset(string file,string display,float duration,float shake,float target)
    {
        ...
        var so=new SerializedObject(p);so.FindProperty("<DisplayName>k__BackingField").stringValue=display;so.FindProperty("<Duration>k__BackingField").floatValue=duration;so.FindProperty("<PreparationCountdown>k__BackingField").floatValue=3f;so.FindProperty("<CameraShakeIntensity>k__BackingField").floatValue=shake;so.FindProperty("<TargetSeconds>k__BackingField").floatValue=target;so.FindProperty("<IntensityCurve>k__BackingField").animationCurveValue=new AnimationCurve(new Keyframe(0,0),new Keyframe(.2f,.35f),new Keyframe(.55f,1),new Keyframe(1,0));so.ApplyModifiedPropertiesWithoutUndo();
    }
    ...
    private static void CreatePrefabs()
    {
        ...
        SavePrefab(BackpackModel("EmergencyBackpack",Vector3.zero),"Assets/Prefabs/Gameplay/EmergencyBackpack.prefab");SavePrefab(RadioModel("EmergencyRadio",Vector3.zero),"Assets/Prefabs/Gameplay/EmergencyRadio.prefab");SavePrefab(KeyModel("AccessKey",Vector3.zero),"Assets/Prefabs/Gameplay/AccessKey.prefab");
        ...
        SavePrefab(ExternalWrapper("KenneyDesk","Assets/ThirdParty/Kenney/FurnitureKit/Models/desk.fbx",Vector3.one,metalMat,new Vector3(2.1f,1.1f,1.1f)),"Assets/Prefabs/Environment/KenneyDesk.prefab");SavePrefab(ExternalWrapper("KenneyChair","Assets/ThirdParty/Kenney/FurnitureKit/Models/chairDesk.fbx",Vector3.one,accentMat,new Vector3(.8f,1.2f,.8f)),"Assets/Prefabs/Environment/KenneyChair.prefab");SavePrefab(ExternalWrapper("KenneyShelf","Assets/ThirdParty/Kenney/FurnitureKit/Models/bookcaseOpen.fbx",Vector3.one,metalMat,new Vector3(1.4f,2.2f,.45f)),"Assets/Prefabs/Environment/KenneyShelf.prefab");SavePrefab(ExternalWrapper("KenneyDoorFrame","Assets/ThirdParty/Kenney/FurnitureKit/Models/doorway.fbx",Vector3.one,wallMat,new Vector3(2.6f,2.8f,.3f)),"Assets/Prefabs/Environment/KenneyDoorFrame.prefab");SavePrefab(ExternalWrapper("KenneyRock","Assets/ThirdParty/Kenney/SurvivalKit/Models/rock-a.fbx",Vector3.one,rockMat,new Vector3(1.2f,.8f,1.2f)),"Assets/Prefabs/Environment/KenneyRock.prefab");SavePrefab(ExternalWrapper("KenneySignpost","Assets/ThirdParty/Kenney/SurvivalKit/Models/signpost.fbx",Vector3.one,safeMat,new Vector3(.5f,2,.5f)),"Assets/Prefabs/Environment/KenneySignpost.prefab");SavePrefab(ExternalWrapper("KenneyRescueTent","Assets/ThirdParty/Kenney/SurvivalKit/Models/tent-canvas.fbx",Vector3.one,safeMat,new Vector3(3,2,3)),"Assets/Prefabs/Environment/KenneyRescueTent.prefab");
    }
    ...
    private static GameObject PlayerObject()
    {
        ...
        var interaction=player.AddComponent<InteractionSystem>();var state=player.AddComponent<GameplayStateController>();Set(state,"playerInput",pi);Set(state,"movement",fpc);Set(state,"starterInputs",inputs);Set(state,"interaction",interaction); return player;
    }
    ...
    private static void BuildLevel(bool level2)
    {
        ...
        var app=new GameObject("App");var sceneLoader=app.AddComponent<SceneLoader>();if(!level2)new GameObject("SimulationSession").AddComponent<SimulationSession>();new GameObject("GameplayCursorController").AddComponent<GameplayCursorController>();
        ...
        EarthquakeController quake=null;if(!level2){var quakeDust=Dust(new Vector3(0,2,10),new Vector3(8,2.5f,18),25);quake=managers.AddComponent<EarthquakeController>();Set(quake,"profile",AssetDatabase.LoadAssetAtPath<EarthquakeProfile>("Assets/ScriptableObjects/Earthquakes/Intermediate.asset"));Set(quake,"cameraEffectRoot",camera.transform);Set(quake,"dust",quakeDust);Set(quake,"objectivesManager",objectives);var quakeUI=canvas.gameObject.AddComponent<EarthquakeUIController>();Set(quakeUI,"earthquakeController",quake);Set(quakeUI,"countdownText",Find(canvas.transform,"EarthquakeCountdown").GetComponent<TextMeshProUGUI>());Set(quakeUI,"intensityText",Find(canvas.transform,"EarthquakeIntensity").GetComponent<TextMeshProUGUI>());}
    ...
    private static Canvas BuildGameplayHud(out ObjectiveUI objectiveUI,out InventoryUI inventoryUI,out NotificationUI notificationUI,out InteractionUIController interactionUI,out GameObject pause,out GameObject completion,out GameCompletionUI completionUI,out GameCompletionController completionController)
    {
        ...
        var timerPanel=Panel("TimerPanel",hud.transform,new Vector2(.5f,.93f),new Vector2(250,62));var timerText=Text("TimerText",timerPanel.transform,"00:00.000",25,new Vector2(.5f,.5f),new Vector2(230,52),Color.white,FontStyles.Bold);var simHud=timerPanel.AddComponent<SimulationHud>();Set(simHud,"timerText",timerText);
        var intensityPanel=Panel("EarthquakeIntensityPanel",hud.transform,new Vector2(.5f,.86f),new Vector2(540,48));Text("EarthquakeIntensity",intensityPanel.transform,"",18,new Vector2(.5f,.5f),new Vector2(510,40),Color.white,FontStyles.Bold);var countdown=Text("EarthquakeCountdown",hud.transform,"",34,new Vector2(.5f,.68f),new Vector2(850,75),Color.white,FontStyles.Bold);countdown.alignment=TextAlignmentOptions.Center;countdown.gameObject.SetActive(false);intensityPanel.SetActive(false);
        ...
        completion=FullPanel("CompletionPanel",screens.transform,"MISIÓN COMPLETADA");var resultsText=Text("ResultsMetrics",completion.transform,"TIEMPO TOTAL",20,new Vector2(.5f,.58f),new Vector2(850,300),Color.white);var results=completion.AddComponent<SimulationResultsUI>();Set(results,"metricsText",resultsText);ButtonObj("RestartButton",completion.transform,"REINICIAR NIVEL",new Vector2(.5f,.47f),null);ButtonObj("MenuButton",completion.transform,"VOLVER AL MENÚ",new Vector2(.5f,.35f),null);ButtonObj("ExitButton",completion.transform,"SALIR",new Vector2(.5f,.23f),null);completionController=completion.AddComponent<GameCompletionController>();completionUI=completion.AddComponent<GameCompletionUI>();Set(completionUI,"controller",completionController);
    ...
    {
        var protection=TriggerMarker("InteriorProtectionZone",new Vector3(-4,1,1.8f),new Vector3(3.5f,2,3));var protectionController=protection.AddComponent<EarthquakeProtectionTrigger>();Set(protectionController,"earthquakeController",quake);SafeZoneVisual(protection.transform,new Vector3(0,-1,0));Sign("ZONA DE PROTECCIÓN",new Vector3(-4,2.6f,2.8f),safeMat);
        ...
        Sign("RUTA DE EVACUACIÓN",new Vector3(0,2.8f,16.5f),safeMat);
    }
    ...
    private static void DressWithExternalAssets(Transform root,bool level2){if(level2){for(int i=0;i<6;i++){var rock=Imported("Assets/Prefabs/Environment/KenneyRock.prefab",root,new Vector3(i%2==0?-6f:6f,.35f,2+i*3),new Vector3(1+i%3*.35f,1,1));rock.name="KenneyRockLandmark";}var sign=Imported("Assets/Prefabs/Environment/KenneySignpost.prefab",root,new Vector3(-3,0,15),Vector3.one);sign.name="KenneyEvacuationSign";var tent=Imported("Assets/Prefabs/Environment/KenneyRescueTent.prefab",root,new Vector3(5,0,20),Vector3.one);tent.name="KenneyRescueTent";}else{var desk=Imported("Assets/Prefabs/Environment/KenneyDesk.prefab",root,new Vector3(-5,0,1.2f),Vector3.one);desk.name="KenneyEmergencyDesk";var chair=Imported("Assets/Prefabs/Environment/KenneyChair.prefab",root,new Vector3(-4.5f,0,2.4f),Vector3.one);chair.name="KenneyOfficeChair";var shelf=Imported("Assets/Prefabs/Environment/KenneyShelf.prefab",root,new Vector3(6.5f,0,6),Vector3.one);shelf.name="KenneySupplyShelf";var doorway=Imported("Assets/Prefabs/Environment/KenneyDoorFrame.prefab",root,new Vector3(6.4f,0,3.5f),Vector3.one);doorway.name="KenneyDoorwayDetail";}}
